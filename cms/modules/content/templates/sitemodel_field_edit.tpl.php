...
<a href="javascript:;" class="on"><em><?php echo L('edit_field');?></em></a><span>|</span><a href="?m=content&c=sitemodel_field&a=init&modelid=<?php echo $modelid?>&menuid=<?php echo $_GET['menuid']?>"><em><?php if($modelid==-1) {echo L('category').L('field_manage');} else if($modelid==-2) {echo L('category_page').L('field_manage');} else if($modelid) {echo L('manage_field');} else {echo L('sites').L('field_manage');}?></em></a><span>|</span></div>
  <div class="bk10"></div>
</div>
...
<option value="/^[0-9]{6}$/"><?php echo L('zip');?></option>
	...
	<tr<?php if(!$modelid || $modelid==-1) {echo ' style="display: none;"';}?>> 
	  ...
	  if(in_array($field,$forbid_fields)) echo 'disabled'; ?>/> <?php echo L('yes');?> <span></span></label>
